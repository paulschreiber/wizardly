...
Here's a list of options for the +act_wizardly_for+ controller macro
  ...
  :suppress_duplicate_warnings_for => [:foo, :bar]
...
The errors would look like this:
  ...
  Mysql2::Error: Duplicate entry '' for key 'index_foos_on_bar': UPDATE `foos` SET `updated_at` = '2011-09-02 16:56:24', `bar` = '' WHERE `id` = 12345)
  
To prevent crashes, list the columns with unique key constrainst on them. For example:

  :suppress_duplicate_warnings_for => [:foo, :bar]

Note that this will not work if you have a nonstandard table or index name.
